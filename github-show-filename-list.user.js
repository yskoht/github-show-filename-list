...
(function () {
  ...
  var addFileNames = (div, fileInfos) => {
    for (var i = 0; i < fileInfos.length; i++) {
      ...
      var srOnly = fileInfo.getElementsByClassName("sr-only")[0];
      if (!srOnly) {
        continue;
      }
      var ariaLabel = srOnly.textContent;
      var m = /^\s*(\d+) changes: (\d+) additions & (\d+) deletions\s*$/.exec(
        ariaLabel
      );
      var addition = m ? m[2] : 0;
      var deletion = m ? m[3] : 0;
